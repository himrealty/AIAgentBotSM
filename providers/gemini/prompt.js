...
(async () => {
  const message = await chrome.storage.local.get('tempMessage');
  const promptText = message.tempMessage || 'Hello';
  ...
  // Store result for background script
  await chrome.storage.local.set({ tempOutput: last });
})();